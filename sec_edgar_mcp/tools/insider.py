...
class InsiderTools:
    ...
    def analyze_form4_transactions(
        ...
        try:
                        ...
                        if hasattr(form4, 'transactions') and form4.transactions:
                            ...
                            for tx in form4.transactions:
                                ...
                                if hasattr(tx, 'price_per_share'):
                                    tx_data['price_per_share'] = float(tx.price_per_share) if tx.price_per_share else None
                                if hasattr(tx, 'transaction_amount'):
                                    tx_data['transaction_amount'] = float(tx.transaction_amount) if tx.transaction_amount else None
                                if hasattr(tx, 'shares_owned_after'):
                                    tx_data['shares_owned_after'] = float(tx.shares_owned_after) if tx.shares_owned_after else None
                        ...
                        if hasattr(form4, 'holdings') and form4.holdings:
                            ...
                            for holding in form4.holdings:
                                ...
                                if hasattr(holding, 'shares_owned'):
                                    holding_data['shares_owned'] = float(holding.shares_owned) if holding.shares_owned else None
            ...
            return {
                "success": False,
                "error": f"Failed to analyze Form 4 transactions: {str(e)}"
            }
    ...
    def analyze_insider_sentiment(
        ...
        try:
            ...
            analysis = {
                ...
                "filing_frequency": "high" if len(recent_filings) > 10 else "low" if len(recent_filings) < 3 else "moderate",
                "recent_filings": []
            }
